...
species="insert name of species for example strep_lacto_competition" 
...
runpytho1(protocol_file,xmlpath,number_repeats) #run the specified protocol file number_repeats times

speciesabud=matrix(0,number_repeats,number_species) #number of columns the number of species
...
for(i in 1:number_repeats){
  ...
  colnames(biovolume)=paste0(colnames(speciesnumber_noEPS(xmlpath1,n,n)),"_biovol")
...
setwd(path1)
write.csv(output_matrix, file = paste0(species,"_results.csv"))
...
type_interaction="competition"
type_interaction="inhibition"
...
#number of species
#combined_dataframe_species is the data frame containing the species number data from 
#the simulations of the single and dual biofilms.
